...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                ...
                <xsl:template match="replaceable[@role='entity']">
                  <xsl:choose>
                                   <xsl:when test=". = 'version'">5.0.0</xsl:when>
                                  <xsl:when test=". = 'shortversion'">5.0</xsl:when>
                                  <xsl:when test=". = 'previousversion'">4.2.x</xsl:when>
                                 <xsl:otherwise>
                ...
                <xsl:param name="target.database.document">/Users/cristinawilliams/testing/alliance/toolkit/docs/5.0/5.0.0/olinkdb.xml</xsl:param>
